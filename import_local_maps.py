            ...
            try:
                ...
                if os.path.exists(read_path):
                    with rasterio.open(read_path) as src:
                        ...
                        # If bounds are in meters (EPSG:3857), convert to degrees using pure math
                        if abs(bounds.left) > 180 or abs(bounds.right) > 180:
                            lon_left, lat_bottom = epsg3857_to_epsg4326(bounds.left, bounds.bottom)
                            lon_right, lat_top = epsg3857_to_epsg4326(bounds.right, bounds.top)

                            leaflet_bounds = [
                                [float(lat_bottom), float(lon_left)],
                                [float(lat_top), float(lon_right)]
                            ]
                        else:
                            leaflet_bounds = [
